...
public class typeTransfer {
    ...
    public static Set<simpleEntity> EntitiesToSimpleEntities(Collection<Entity> entities) {
        ...
        for (Entity entity:entities) {
            simpleEntities.add(entityToSimpleEntity(entity));
        }
        return simpleEntities;
    }

    public static simpleEntity entityToSimpleEntity(Entity entity) {
        return typeTransfer.transferEntity(entity);
    }

    private static simpleEntity transferEntity(Entity entity) {
        Class<? extends Entity> clazz=entity.getClass();
        switch (clazz.toString()) {
            case useRedis.DeploymentClass:
                return new deployment((DeploymentEntityImpl)entity);
            case useRedis.ExecutionClass:
                return new execution((ExecutionEntityImpl)entity);
            case useRedis.ProcessDefinitionClass:
                return new processDefinition((ProcessDefinitionEntityImpl)entity);
            case useRedis.ResourceClass:
                return new resource((ResourceEntityImpl)entity);
            case useRedis.TaskClass:
                return new task((TaskEntityImpl)entity);
            case useRedis.VariableClass:
                return new variable((VariableInstanceEntityImpl)entity);
            case useRedis.PropertyClass:
                return new property((PropertyEntityImpl)entity);
        }
        return null;
    }
}
